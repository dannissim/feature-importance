...
class WaveletsForestRegressor:
    ...
    def __init__(self,
                 ...
                 trees=5,
                 seed=2000,
                 norms_normalization='num_samples',
                 train_vi=True,
                 vi_threshold=0.8,
                 max_m_terms=1000):
        '''
        ...
        for i in range(len(rf.estimators_)):
            ...
            if self.train_vi:
                for k in range(0, num_features):
                    ...
                    if len(self.si) == 0:
                        self.si = np.sum(self.si_tree, 1) / len(rf.estimators_)
                    else:
                        self.si = self.si + np.sum(self.si_tree, 1) / len(rf.estimators_)
    ...
    def __compute_norm(self, avg, parent_avg, volume):
        norm = np.power(
            np.sum(np.power(np.abs(avg - parent_avg), self.power)) * volume, (1 / self.power))
        return norm
    ...
    def evaluate_angle_smoothness(self,
                                  text='',
                                  output_folder='',
                                  epsilon_1=None,
                                  epsilon_2=None,
    ...
    def predict(self, X, start_m=0, paths=None):
        '''
		...
		:paths: Instead of computing decision paths for each sample, the method can receive the indicator matrix.
		 Can be used to evaluate predictions incrementally over terms.
		:return: Predictions.
		'''
        X = X.values
        cur_sorted_norms = self.sorted_norms[start_m:self.max_m_terms]
        if paths == None:
    ...
    def __variable_importance(self, estimator, base_node_id, vi_node_box, vi_norms, vi_vals,
                              feature, threshod):
        if base_node_id == 0:
        ...
        if left_id >= 0:
            ...
            self.__variable_importance(estimator, left_id, vi_node_box, vi_norms, vi_vals, feature,
                                       self.vi_threshold)
            vi_vals[:, left_id] = estimator.tree_.value[left_id][:, 0] - estimator.tree_.value[
                base_node_id][:, 0]
            tnorm = self.__compute_norm(vi_vals[:, left_id], vi_vals[:, base_node_id], 1)
            if estimator.tree_.feature[
                    estimator.tree_.children_left[base_node_id]] == feature and tnorm > threshod:
        ...
        if right_id >= 0:
            ...
            self.__variable_importance(estimator, right_id, vi_node_box, vi_norms, vi_vals, feature,
                                       self.vi_threshold)
            vi_vals[:, right_id] = estimator.tree_.value[right_id][:, 0] - estimator.tree_.value[
                base_node_id][:, 0]
            tnorm = self.__compute_norm(vi_vals[:, right_id], vi_vals[:, base_node_id], 1)
            if estimator.tree_.feature[
                    estimator.tree_.children_right[base_node_id]] == feature and tnorm > threshod:
